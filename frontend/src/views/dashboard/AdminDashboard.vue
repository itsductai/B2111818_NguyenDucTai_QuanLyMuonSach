...
const stats = ref([
  ...
  { label: 'Tổng số nhân viên', value: 0, icon: 'fa-user-tie' },
...
onMounted(async () => {
  try {
    ...
    stats.value[2].value = staff.nhanVien?.length || 0;
    stats.value[3].value = borrows.theoDoiMuonSach?.length || 0;
    
    // Lấy phiếu mượn chờ duyệt
    const pendingBorrowsData = await muonSachService.getMuonSachs({ tinhTrang: 'Chưa duyệt' });
